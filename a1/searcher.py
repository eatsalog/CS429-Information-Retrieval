...
class Index(object):
    ...
    def create_champion_index(self, index, threshold=10):
        ...
        for i in index:
            n[i] = sorted(index[i], key = lambda x: x[1], reverse = True)[:threshold]
            ...
            for d in docs:
                ...
                for x in d:
                    if x == i:
                        tf += 1
                if tf > 0:
                    n[i].append([num, float(tf)*math.log(len(docs)/doc_freqs[i], 10)])
                num += 1
        res = {}
        for c in n:
            if n[c] != []:
    ...
    def count_doc_frequencies(self, docs):
        ...
        for i in docs:
            d = []
            for x in i:
                if x in n and x not in set(d):
                    n[x] += 1
                    d.append(x)
                else:
                    if x not in n and x not in set(d):		
                        n[x] = 1
                        d.append(x)
        return n
    ...
    def query_to_vector(self, query_terms):
        ...
        for term in query_terms:
            if term in self.index:
                n[term] = math.log(len(self.documents)/self.doc_freqs[term],10)
        return n
    ...
    def search_by_cosine(self, query_vector, index, doc_lengths):
        ...
        for t, w in query_vector.items():
            for did, dw in index[t]:
                scores[did] += w*dw

        for did in scores:
            scores[did] /= doc_lengths[did]
    ...
    def search(self, query, use_champions=False):
        ...
        vector = self.query_to_vector(token)
        champions = create_champion_index(self.index, threshold = 10)

        if use_champions == True:
            searching = self.search_by_cosine(vector, champions, self.doc_lengths)
        else:
            searching = self.search_by_cosine(vector, self.index, self.doc_lengths)
        
        doc_ids = []
        for ids, score in searching.items():
            doc_ids.append(ids)
