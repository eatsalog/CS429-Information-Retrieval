...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        for term in index:
            for value in range(len(index[term])):
                l[index[term][value][0]] += ((1+math.log10(index[term][value][1]))*(math.log10(n_docs/doc_freqs[term])))**2
        
        for yes in l:
            l[yes] = math.sqrt(l[yes])
