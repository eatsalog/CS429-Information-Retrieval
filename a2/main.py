...
def parse_relevance_strings(strings):
    ...
    for term in strings:
        if len(term) == 0 or term.isspace():
            continue
        for doc in term.split()[1:]:
             sdict[int(term.split()[0])].append(int(doc))
...
def parse_query_strings(strings):
    ...
    for term in strings:
        if len(term) == 0:
            continue
        elif term.startswith('*FIND'):
            words = term.split()
            doc = int(words[-1])
            continue
        elif term.startswith('*STOP'):
            break
        else:
            if len(diction[doc]) > 0:
                diction[doc] += ' '
            diction[doc] += term
...
def search(query, scorer, index):
    ...
    """
    token = index.tokenize(query)
    quering_vector = index.query_to_vector(token)
    scoring = scorer.score(quering_vector, index)
