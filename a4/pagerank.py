...
def compute_pagerank(urls, inlinks, outlinks, b=.85, iters=20):
    ...
    The formula is R(u) = (1/N)(1-b) + b * (sum_{w in B_u} R(w) / (|R_w|)
    ...
    for i in range(iters):
        for u in urls:
            s = 0.0
            for w in inlinks[u]:
                s += R_u[w]/len(outlinks[w])
        
            R_u[u] = (1 - b)/len(urls) + b * s
...
def read_links(path):
    ...
    read = read_names(path)

    for name in read:
        html = open(path+os.sep+name, encoding="utf8").read()
        links = get_links(read, html)
